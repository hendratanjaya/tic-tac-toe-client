...
export async function fetchHistory(session_id){
    ...
    try{ 
        const response = await fetch(`https://tic-tac-toe-server-hendratanjaya.vercel.app/api/history?API_KEY=${apikey}`);
        if(response.ok){
...
export async function addNewGame(newHistory){
    ...
    try {
        const response = await fetch(`https://tic-tac-toe-server-hendratanjaya.vercel.app/api/add-new-game?API_KEY=${apikey}`, {
            method: 'POST', 
...
export async function updateHistory(activeGame, idx){
    ...
    const apikey = localStorage.getItem("session_id");
    try {
        const response = await fetch(`https://tic-tac-toe-server-hendratanjaya.vercel.app/api/update-history?API_KEY=${apikey}`, {
            method: 'POST', 
            headers: {'Content-Type': 'application/json'},
